...
<header>
	...
	<section class="gallery py-md-5">
		...
		<h2 class="tittle text-center mb-3">活动</h2>
						
				<p class="tit text-center mx-auto">仅支持通过活动名称，活动开始时间（如2020-01-02）进行搜索，多个搜索条件请用空格隔开</p>
			<div class="container">
<div class="input-group">
<input type="text" class="form-control input-lg" id="seaContext"><button onclick=searchAct(); class="input-group-addon btn btn-primary">搜索</button>
</div>
</div>
			...
			<div id="con" class="row gallery-info pt-4 mt-md-4">
				<c:forEach items="${activities }" var="act">
					...
					<a href="statics/images/g${act.getAId()%12 }.jpg" class="gallery-box" data-lightbox="example-set" data-title="">
						<img src="statics/images/g${act.getAId()%12 }.jpg" alt="" class="img-responsive zoom-img">
					</a>
				</div>
				<div class="col-md-3 gallery-grids" style="position:relative;">
						<p>名称：${act.getAName() }</p>
						<p>描述：让我们一起享受这次旅行——${act.getADescription() }</p>
						<p>开始时间：${act.getAStarttime() }</p>
						<p>预计结束时间：${act.getAEstimateendtime() }</p>
					<a href="javascript:doPost('detail','detail','${act.getAId()}')"><button class="input-group-addon btn btn-primary" style="position: absolute;bottom:0;right:0" >详情</button></a>
				</div>
